...
<c:set var="dateNumber" value="${dateNumber}"/>
<input type="hidden" id="dateNumberId" value="${dateNumber}"/>
<input type="hidden" id="pageId" value="${pageNumber}"/>
<input type="hidden" id="noOfOrdersResult" value="${searchPageData.pagination.totalNumberOfResults}"/>           
<input type="hidden" id="resultCount" value="${counter}"/>

<c:set var="searchUrl" value="/my-account-voice/ordersVoice?dateNumber=${dateNumber}&sort=${ycommerce:encodeUrl(searchPageData.pagination.sort)}"/>

<%-- <c:choose>
	<c:when test="${dateNumber != 0}">
		<c:set var="searchUrl" value="/my-account/orders?dateNumber=${dateNumber}&sort=${ycommerce:encodeUrl(searchPageData.pagination.sort)}"/>
	</c:when>
	<c:otherwise>
