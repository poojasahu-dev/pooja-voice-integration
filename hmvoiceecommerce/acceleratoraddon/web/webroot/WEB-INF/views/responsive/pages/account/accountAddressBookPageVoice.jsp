	    ...
	    <div class="account-cards card-select">
			<div class="row">
				<c:forEach items="${addressData}" var="address">
					...
					<c:set var="counter" value="${counter+1}" scope="request"></c:set>
						<ul class="pull-left">
						...
						<c:if test="${not address.defaultAddress}">
							<ycommerce:testId code="addressBook_isDefault_button">
								<a class="account-set-default-address" id="setAsDefaultAddress_${counter}" href="set-default-address/${fn:escapeXml(ycommerce:encodeUrl(address.id))}">
						...
						<div class="account-cards-actions pull-left">
							<ycommerce:testId code="addressBook_editAddress_button">
								<a class="action-links" id="editAddress_${counter}" href="edit-address/${fn:escapeXml(ycommerce:encodeUrl(address.id))}">
									<span class="glyphicon glyphicon-pencil"></span>
								</a>
							</ycommerce:testId>
							<ycommerce:testId code="addressBook_removeAddress_button">
								<a href="#" class="action-links removeAddressFromBookButton" id="removeAddress_${counter}"
								data-address-id="${fn:escapeXml(address.id)}" data-popup-title="<spring:theme code="text.address.delete.popup.title" />">
                                    ...
                                    <ycommerce:testId code="addressRemove_delete_button">
                                        <div class="col-xs-12 col-sm-6 col-sm-push-6">
                                            <a class="btn btn-primary btn-block" data-address-id="${fn:escapeXml(address.id)}"
                                             id="deleteAddress" href="remove-address/${fn:escapeXml(ycommerce:encodeUrl(address.id))}">
