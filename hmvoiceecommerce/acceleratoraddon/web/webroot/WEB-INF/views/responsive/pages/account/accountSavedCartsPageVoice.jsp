...
<%@ taglib prefix="cart" tagdir="/WEB-INF/tags/addons/hmvoiceecommerce/responsive/cart" %>
...
<c:if test="${not empty searchPageData.results}">
    <div class="account-section-content">
        ...
        <input type="hidden" id="noOfSavedCarts" value="${searchPageData.pagination.totalNumberOfResults}"/>
        <input type="hidden" id="voiceId" value="${isVoice}"/> 
                ...
                <c:forEach items="${searchPageData.results}" var="savedCart" varStatus="loop">
                    ...
                    <tr id="row-${loop.index}" class="responsive-table-item">
          				<td class="hidden-sm hidden-md hidden-lg">
          				 ...
          				 <c:set var="name" value="${fn:escapeXml(savedCart.name)}"/>
						 <c:set var="nameL" value="${fn:toLowerCase(name)}"/>
						 <c:set var="nameTrim" value="${fn:replace(nameL,' ','')}"/>
